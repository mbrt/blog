---
title: "Creating the AI from my childhood comics"
date: 2026-02-28
tags: [ai]
toc: true
---
...
I approached the problem in two phases:[^2]
...
![](pkna-uno-extract.excalidraw.svg)
...
to try and counteract hallucinations[^3] and keep the assistant-like behavior intact:
...
shitty code) from [Disney Comics Fandom](https://disney-comics.fandom.com/),[^5] and a [rewrite
...
clearly evolved by trial and error, in combination with vanilla [Llama 3](https://www.llama.com/models/llama-3/):[^6]
...
to the model to generate something "random".[^7]
...
It's truly magic to see the traces;[^8] Additionally, the runtime and cost were definitely lower than the "feed one
...
[^2]:  There was actually a phase zero, scanning the comics, which I did many years ago when I had time to spare, and was afraid they would get lost. The scans turned out to be useful in a different way.

[^3]:  I know that telling the model not to hallucinate doesn’t really work in practice, but it’s always worth trying.

[^4]:  "Final" at the time of writing.

[^5]:  I believe the [CC BY-SA](https://www.fandom.com/licensing) license allows for such usage.

[^6]:  Yes, there are other elements, such as an "[emotional state system](https://deepwiki.com/dnhkng/GLaDOS/3.4-emotional-state-system)", but the core of the personality is defined in that simple prompt.

[^7]:  I put "random" in quotes here, as models are notoriously incapable of truly diversified random outputs.

[^8]:  I recommend setting `verbose=True` specifically for this reason.
